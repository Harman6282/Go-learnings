...
type TaskManagerClient interface {
	// unary: create single task
	CreateTask(ctx context.Context, in *CreateTaskRequest, opts ...grpc.CallOption) (*CreateTaskResponse, error)
	// server streaming: list all tasks as a stream
	ListTasks(ctx context.Context, in *ListTaksRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Task], error)
}
...
func (c *taskManagerClient) ListTasks(ctx context.Context, in *ListTaksRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Task], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &TaskManager_ServiceDesc.Streams[0], TaskManager_ListTasks_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ListTaksRequest, Task]{ClientStream: stream}
...
type TaskManagerServer interface {
	...
	// server streaming: list all tasks as a stream
	ListTasks(*ListTaksRequest, grpc.ServerStreamingServer[Task]) error
...
func (UnimplementedTaskManagerServer) ListTasks(*ListTaksRequest, grpc.ServerStreamingServer[Task]) error {
...
func _TaskManager_ListTasks_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ListTaksRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(TaskManagerServer).ListTasks(m, &grpc.GenericServerStream[ListTaksRequest, Task]{ServerStream: stream})
}
